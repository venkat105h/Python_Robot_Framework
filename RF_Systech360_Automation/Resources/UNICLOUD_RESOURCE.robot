...
UNICLOUD_Login to UniCloud using URLS and Username and Password parameters
...
UNICLOUD_Logout of UniCCloud session
...
UNICLOUD_Systech UniCCloud Should Be Open
...
UNICLOUD_Navigation to Add Customer Pages From Welcome Page
    [Documentation]     This keyword is to Navigate to Add Customer page from Welcome page.
    ${url}=    get text      (//*[@id="preferencecard-name"])[1]
    ${count}=    get text      (//*[@id="preferencecard-name"])[2]
    log    ${count}
    log    ${url}
...
UNICLOUD_Click on Environment Record in the First page
...
UNICLOUD_Click on Environment Records in All pages
...
UNICLOUD_Click on Environment Records
    ...
    Run Keyword If  '${Pagination_Status}'=='${FALSE}'   UNICLOUD_Click on Environment Record in the First page   ${ENVIRONENTID}
	Run Keyword If  ${Paginationlinks} == 5 and '${Pagination_Status}'=='${TRUE}'  UNICLOUD_Click on Environment Record in the First page   ${ENVIRONENTID}
	Run Keyword If  ${Paginationlinks} > 5 and '${Pagination_Status}'=='${TRUE}'   UNICLOUD_Click on Environment Records in All pages    ${ENVIRONENTID}  ${Paginationlinks}  ${RowCount}
...
UNICLOUD_Select the Account Disable Checkbox
	...
	UnSelect Checkbox    ${UNICLOUD_DISACCOUNT_CHKBOX}
	Select Checkbox   ${UNICLOUD_DISACCOUNT_CHKBOX}
...
UNICLOUD_Select the options in the add feature
    ...
    ...            ${UNICLOUD_FEATURE_OPT}    ${UNICLOUD_SYSTEMRESPONSE_OPT}
	...
	...   AND   UNICLOUD_Click on Save button on added feature   ${j}
...
UNICLOUD_Click on Save buttton
	...
	Click Element  ${UNICLOUD_ADDCUSTOMER_SAVEBUTTON}
	Wait Until Element Is Visible  ${UNICLOUD_SAVEMSG_MSGBOX}  timeout=20
