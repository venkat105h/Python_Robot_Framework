...
SPH_DESN_Click on VA Play button
    ...
    Wait Until Element Is Visible  ${UNISPHERE_VIDEO_SECTION}  timeout=20
#-----------------------------------------------------------------------------------------------------------------------
SPH_DESN_Check the Online Video Assistance window
    ...
    Wait Until Element Is Visible  ${UNISPHERE_VIDEO_SECTION}  timeout=20
    Page Should Contain Element  ${UNISPHERE_VIDEO_SECTION}
...
SPH_DESN_Click on Save button in Editor page
    ...
    #Mouse Over   ${UNISPHERE_FLOATBTNICON_ID}  #icon is removed
    ${is_message_present} =    run keyword and return status    wait until element is visible    ${UNISPHERE_MESSAGECENTERCLOSE_BTN}    timeout=3s
    run keyword if    ${is_message_present}==True    Wait Until Keyword Succeeds     5x    2s    Click Element    ${UNISPHERE_MESSAGECENTERCLOSE_BTN}
    wait until keyword succeeds  15x  2s  Click Element    ${UNISPHERE_SAVEBTN_ID}
    run keyword and ignore error    wait until element is visible    ${UNISPHERE_MESSAGECENTERCLOSE_BTN}
    run keyword and ignore error    Click Element    ${UNISPHERE_MESSAGECENTERCLOSE_BTN}
#-----------------------------------------------------------------------------------------------------------------------
...
SPH_DESN_Search the new definition after save in Table view
    ...
    Wait Until Element Is Visible  ${UNISPHERE_TABLE_ROWS}/label  timeout=20
    Page should contain element    ${UNISPHERE_TABLE_ROWS}/label
#-----------------------------------------------------------------------------------------------------------------------
SPH_DESN_Search the new definition after save and Click the Item
    ...
    FOR  ${Index}  IN RANGE  1  20
       wait until keyword succeeds  5x  2s    Wait Until Element Is Visible    ${Def_Item}_itemName_0
       ${name}=  wait until keyword succeeds  5x  2s    Get Text  ${Def_Item}_itemName_0
...
SPH_DESN_Drag and Drop the subordinates
    ...
    Wait Until Element Is Visible    ${UNISPHERE_AVAI_ITEM_LISTS}  timeout=20
    Click Element   ${UNISPHERE_AVAI_ITEM_SERACH_BTN}
    Click Element  ${UNISPHERE_AVAI_ITEM_LISTS}
    Click Element  ${UNISPHERE_SELECT_SINGLE_ITEM_BTN}
...
SPH_DESN_Search and Add the Subordinates object
    ...
    #Scroll Page To Location    0    2000
    Wait Until Element Is Visible   ${UNISPHERE_AVAI_ITEM_SEARCHBOX}  timeout=20
    Wait Until Element Is Visible    ${UNISPHERE_AVAI_ITEM_SERACH_BTN}  timeout=20
    Wait Until Element Is Visible    ${UNISPHERE_AVAI_ITEM_LISTS}  timeout=20
    press keys      ${UNISPHERE_AVAI_ITEM_SEARCHBOX}   ${SubOrdinatename}
    Click Element   ${UNISPHERE_AVAI_ITEM_SERACH_BTN}
    Click Element  ${UNISPHERE_AVAI_ITEM_LISTS}
    Click Element  ${UNISPHERE_SELECT_SINGLE_ITEM_BTN}
#-----------------------------------------------------------------------------------------------------------------------
SPH_DESN_Add Multiple Subordinates object
    ...
    #SPH_DESN_Click on Show Subordinate
    ${SubOrdinateCount}=  Get Element Count  ${UNISPHERE_AVAI_ITEM_LISTS}
    log to Console   ${SubOrdinateCount}
...
SPH_DESN_Remove the Subordinate object
    ...
    Wait Until Element Is Visible    ${UNISPHERE_SELECTED_ITEM_LISTS}  timeout=20
    Click Element  ${UNISPHERE_SELECTED_ITEM_LISTS}
    Click Element  ${UNISPHERE_REMOVE_SINGLE_ITEM_BTN}
#-----------------------------------------------------------------------------------------------------------------------
...
SPH_DESN_Check the added subordinates are displayed in Graphical tab
    ...
    Wait Until Element Is Visible   ${UNISPHERE_AVAILABLE_TITLE_CLASS}  timeout=20
    ${ChildCount}=  Get Element Count  ${UNISPHERE_INCLUDED_TITLE_CLASS}
    log to Console   ${ChildCount}
    ${ChildList}=  Create List
    FOR  ${k}  IN RANGE   1    ${ChildCount}
         Run Keyword If  ${k} == ${ChildCount}    exit for loop
         ${values}=    Get Text     ${UNISPHERE_INCLUDED_TITLE_CLASS} [${k}]
...
SPH_DESN_Click on Accept/Reject Button
    ...
    [Arguments]  ${Button}
    wait until keyword succeeds  2x   2s    Wait Until Element Is Visible    ${Button}
    wait until keyword succeeds  2x   2s    Mouse Over   ${Button}
    wait until keyword succeeds  5x   2s    Click Element   ${Button}
    Wait Until Element Is Visible   ${UNISECURE_MESSAGE_ID}  timeout=60
    wait until keyword succeeds  3x   2s  Click Element  ${UNISECURE_CLOSE_MSG_CENTER}
#-----------------------------------------------------------------------------------------------------------------------
...
SPH_DESN_Check the Graphical tab Elements
    ...
    FOR  ${i}  IN  @{GraphicalElements}
         Wait Until Element Is Visible   //*[@id="${i}"]  timeout=60
